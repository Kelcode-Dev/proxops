...
// (PVE `template=1`). It is a first-class pveconform kind as of M11.
//
// Wire shape: identical to a pveconform VM. A TemplateVM embeds a VM and
...
//     VM cannot claim the same (node, vmid). pveconform enforces this at
...
//   - A PVE template CANNOT be started. pveconform's TemplateVM DesiredState()
//     is always "stopped"; spec.state "started" fails at parse time.
//   - PVE 9.2 has no /qemu/{id}/untemplate endpoint (probe-verified
//     HTTP 501 "not implemented"). pveconform therefore:
...
//       * and — when a pveconform VM (kind=VM) is desired but PVE's live
...
//
//   - pveconform adopt emits a TemplateVM manifest for any PVE object that
//     reports template=1. M10's "skip + census" behaviour is REPLACED in
//     M11: the manifest is written under templatevm/<cluster>/, the
//     pveconform ownership tag + every owned VM field is captured via the
...
//     apply. pveconform's Drift treats the sentinel as "PVE owns live
//     sshkeys — do not write", so round-trips are stable.
//
// Dependency semantics: a TemplateVM's only pveconform-side edge is the
// inherited VM cdrom.iso edge (to an ISO). pveconform does NOT model a
// "VM clones TemplateVM" edge in M11 — VMs are never created by PVE clone
// from a pveconform TemplateVM (a VM manifest describes a fresh VM, disks
// allocated by PVE at create, which is the pveconform guarantee against
...
// key, which is not what M11 manifests carry — the pveconform manifest
// for a TemplateVM is syntactically identical to a pveconform VM
